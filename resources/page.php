...
class page {
    ...
    public function getLibrary($org ='',$offset = 0){
        ...
        try {
            ...
            switch ($org){
                ...
                default:
                    $stmt = $db->prepare("SELECT * FROM comicNew WHERE user_id = :userid GROUP BY comic_series, comic_start_year ORDER BY comic_series LIMIT 16 OFFSET :offsetValue");
    ...
    public function getComicSeries($seriesid, $offset = 0, $asArray = false){
        ...
        try {
            ...
            $stmt = $db->prepare("SELECT * FROM comicNew WHERE user_id = :userid AND comic_series_id = :seriesid ORDER BY comic_series LIMIT 16 OFFSET :offsetValue");
            ...
            if (!empty($result)){
                ...
                foreach($result as $element) {
                    ...
                    $comicResults .= "<a class='$process' data-comic-id='".$element['comic_id']."' data-series-name='".$element['comic_series_name']."' href='comic.php?id=".$element['comic_id']."'><div class='comicCard'><img src='image.php?id=".$element['comic_id']."&page=0'/><p> #".$element['comic_issue']."</p></div></a>";
                }
                return $comicResults;
            }else{
                return $comicResults = "<p id='noResults'>No Results Found</p>";
...
class ComicViewer extends page{
    ...
    public function getComic($comicID,$asArray = false){
        ...
        try {
            ...
            $stmt = $db->prepare("SELECT * FROM comicNew WHERE comic_id= :value");
